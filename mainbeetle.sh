...
# folders and files ready for PAGAN
mkdir Flyensemblalign
mkdir Flygapless
mv Flies/*.fasta Flyensemblalign

# remove gaps to prepare the files for PAGAN
find Flyensemblalign -name 'fly*.fasta' -print0 | xargs -0 -I {} sh -c 'base=$(basename $1) ; name=${base%.*} ; ext=${base##*.} ; python /home/lauralaa/Scripts_beetle/rmgaps.py "$1" "Flygapless/${name}.fasta"' -- {}

# align everything with PAGAN, put them back to the original folder with original names
find Flygapless -name 'fly*.fasta' -print0 | xargs -0 -I {} sh -c 'base=$(basename $1) ; name=${base%.*} ; ext=${base##*.} ; pagan --seqfile "$1" --treefile "Flies/${name}.tree" --outfile "Flies/${name}" --translate' -- {}

# change PAGAN generated extensions back to .fasta
cd Flies
rename 's/\.codon.fas/\.fasta/' ./*.codon.fas
rm *.fas
cd ..
