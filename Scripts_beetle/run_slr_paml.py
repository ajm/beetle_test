...
def paml(configfile, resultfile) :
    ...
    # then some folder tricks so you can run many pamls at the same time
    pwd = getcwd()
    name = configfile.replace("_.config", "")
    ...
    try :
        shutil.copy2("%s_.config" % name, tmpfolder)
	shutil.copy2("%s_.phy" % name, tmpfolder)
	shutil.copy2("%s_.tree" % name, tmpfolder)
    except OSError :
        pass
    ...
    print "\tgot paml results"

    os.chdir(pwd)

    try :
        pamlfile = "%s/%s_.paml" % (tmpfolder, name)
        shutil.copy2(pamlfile, ".")
    except OSError :
	pass
...
def slr(configfile, resultfile) :
    ...
    if not isfile(resultfile) :
        print "Got no Slr results for %s" % resultfile.replace("_", "")
...
def main() :
    ...
    if len(argv) != 3 :
        print >> stderr, "Usage: %s <msa> runraxml?(y/n)" % argv[0]
        exit(1)
    ...
    # making sure that things don't go wrong if there are files left from a previous run ...
    # they would have "_." in the name already
    # otherwise filenames should never end with "_.", only with numbers
    if "_." in msa :
        return 0
